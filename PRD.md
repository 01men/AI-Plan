...
| 模块 | 功能 | 状态 | 负责 |
|---|---|---|---|
| 组织 | 5 平台 28 部门 48 人组织树视图、梯队角色徽标 | ✅ | Kimi |
...
| 效能度量 | 八项 KPI 驾驶舱、双口径覆盖率、ROI 57.5%（方案口径静态展示值，随运营数据积累切换实测）、投入构成方案口径、心跳日报 | ✅ | Kimi |
...
| Multica 桥接 | 外部运行时事件回传（幂等）、按 ID 取任务、驳回保持待重派（契约级测试 5/5 通过；真实 CLI 端到端联调待环境，见 B-2） | ✅ | GPT |
